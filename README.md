...
子页面卡片的位置由腾讯文档决定，插件不会移动卡片。正文是贴着普通内容块插入的（接口只认内容块，卡片本身不能当插入点），所以：卡片后面跟着普通内容时，正文正好排在卡片下方；两张卡片紧挨在一起、或卡片是页面第一块时，那一侧没有可插入的位置，插件会把正文排到能放的地方并提示你拖动卡片；卡片是文档最后一块时，正文追加到文档末尾，也就是卡片正下方。

写入位置只能靠页面内部的普通内容块定位（接口没有页面参数，`<Page>` 块也不能当锚点），因此新建的空子页面需要先在腾讯文档里输入任意一个字符作为占位，插件才能把内容写进去并自动删除该占位；没有占位的子页面会在发布时被跳过并给出提示，不会把内容错写到根页面。
...
“最近”会从最近文件中分页筛选最多 20 篇腾讯智能文档，表格、Word、PDF 等其他类型不会显示。绑定要求本地与远端父子层级一致；若层级不同，请调整 Obsidian 链接树或腾讯子页面结构后刷新。
...
自动化测试包括 8 个测试文件、67 个测试用例，并覆盖两层页面树、腾讯响应省略当前页外壳且仅含一个子页面的情况、卡片两侧的正文分段、相邻卡片与页面首尾卡片、根页面追加、子页面锚点写入与空子页面跳过、含花括号的 Markdown/LaTeX 数学公式、超过 20 个 Block、最近文档分页、远端冲突、恢复路径、JSON/SSE MCP 响应、图片和 PDF。发布前仍需在真实 Obsidian Vault 与腾讯测试文档中执行人工验收。

## 发布

最新测试版为 [`0.1.10`](https://github.com/totok22/tencent-docs-publisher/releases/tag/0.1.10)，可通过 BRAT 安装或更新。
