...
子页面卡片的位置由腾讯文档决定：插件会把正文排在卡片周围，但接口不允许在两张相邻卡片之间插入内容。想让某张卡片出现在正文下方，请在腾讯文档里把它拖到目标位置，插件会按新位置发布。
...
自动化测试包括 8 个测试文件、62 个测试用例，并覆盖两层页面树、腾讯响应省略当前页外壳且仅含一个子页面的情况、根页面追加、子页面锚点写入与空子页面跳过、含花括号的 Markdown/LaTeX 数学公式、超过 20 个 Block、最近文档分页、远端冲突、恢复路径、JSON/SSE MCP 响应、图片和 PDF。发布前仍需在真实 Obsidian Vault 与腾讯测试文档中执行人工验收。

## 发布

最新测试版为 [`0.1.8`](https://github.com/totok22/tencent-docs-publisher/releases/tag/0.1.8)，可通过 BRAT 安装或更新。
