...
自动化测试包括 10 个测试文件、76 个测试用例，并覆盖两层页面树、腾讯响应省略当前页外壳且仅含一个子页面的情况、卡片两侧正文的分段、相邻卡片与页面首尾卡片、根页面追加、子页面锚点写入与空子页面跳过、Markdown/LaTeX 数学公式、图片尺寸语法与六种图片格式的等比高度、远端冲突、恢复路径、JSON/SSE MCP 响应、图片和 PDF。发布前仍需在真实 Obsidian Vault 与腾讯测试文档中执行人工验收。

## 发布

最新测试版为 [`0.1.11`](https://github.com/totok22/tencent-docs-publisher/releases/tag/0.1.11)，可通过 BRAT 安装或更新。
